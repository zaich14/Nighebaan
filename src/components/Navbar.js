...
function Navbar() {
  ...
  const patientNav = (
    <>
      ...
      <Link to="/alerts"           className={`${navLink} ${isActive("/alerts")           ? active : inactive}`}>Alerts</Link>
    ...
    <nav className="sticky top-0 z-40 bg-white shadow-sm border-b border-slate-200">
      <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-4">
          ...
          {/* Logo */}
          <Link to={user ? dashboardLink() : "/"} className="flex items-center gap-2.5 shrink-0 transition hover:opacity-80">
            <img src="/logo.jpeg" alt="Nigehbaan Logo" className="h-9 w-9 rounded-lg object-cover" />
            ...
            {user ? (
              <>
                <div className="flex items-center gap-2 rounded-2xl bg-slate-50 border border-slate-200 px-3 py-1.5">
                  <div className="flex h-6 w-6 items-center justify-center rounded-full bg-teal-600 text-white text-xs font-bold shrink-0">
                    {user.name?.charAt(0).toUpperCase() || "?"}
                  </div>
                  <span className="text-sm font-medium text-slate-700 max-w-[100px] truncate hidden sm:block">
                    {user.name}
                  </span>
                  <span className="rounded-full bg-teal-600 px-2 py-0.5 text-xs font-semibold text-white hidden sm:block">
                    {roleLabel(user.role)}
                  </span>
                </div>
                <button
          ...
          <div className="lg:hidden mt-3 border-t border-slate-100 pt-3 pb-2 flex flex-col gap-3">
            ...
            {user && isPatient && (
              <>
                ...
                <Link to="/alerts"           className={`${navLink} ${inactive}`}>Alerts</Link>
